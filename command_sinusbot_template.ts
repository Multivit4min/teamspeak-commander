...
import { chatEvent } from "@sinusbot/modules/event";
...
registerPlugin({
...
}, (_, { DEBUG_LEVEL, NOT_FOUND_MESSAGE }, { version }) => {
    ...
    function DEBUG(level: DEBUGLEVEL) {
        return (mode: DEBUGLEVEL) => (...args: any[]) => {
    ...
    const debug = DEBUG(parseInt(DEBUG_LEVEL, 10))
    ...
    debug(DEBUGLEVEL.VERBOSE)(`command prefix is "${getCommandPrefix()}"`)
    ...
    class TooManyArguments extends Error {
        ...
        constructor(err: string, parseError: ParseError|null) {
            super(err)
    ...
    abstract class Argument {
        ...
        private _displayDefault: boolean
         ...
         */
        optional(fallback: any, displayDefault:boolean = true): Argument {
    ...
    class StringArgument extends Argument {
        ...
        private _minlen: number|null = null
         ...
         * @returns {string[]} Array containing two strings: argument and the rest
         */
        _validate(arg: string, ...rest: string[]) {
    ...
    class NumberArgument extends Argument {
        ...
        private _forcePositive: boolean = false
        private _forceNegative: boolean = false
        ...
        validate(args: string) {
            ...
            if (isNaN(arg) || arg === undefined) throw new ParseError(`"${arg}" is not a number"`, this)
    ...
    class CommandCollector {
         ...
         */
        static validateCommandName(name: string, allowSingleChar = true) {
         ...
         */
        getAvailableCommandsWithPrefix(cmd: string) {
     ...
     */
    class Throttle {
        private _throttled: Record<string, ThrottleInterface> = {}
        private _penaltyPerCommand = 1
        private _initialPoints = 1
        private _restorePerTick = 1
        private _tickrate = 1000
        ...
        private refreshTimeout(id: string) {
            ...
            this._throttled[id].timeout = setTimeout(this.restorePoints.bind(this, id), this._tickrate)
        ...
        private createIdIfNotExists(id: string) {
            ...
            this._throttled[id] = { points: this._initialPoints, next: 0 }
    ...
    class Command {
        ...
        private _throttle: Throttle|null = null
        private _args: Array<Argument> = []
        private _manual: Array<string> = []
        private _fncs: Record<string, Function> = {}
         ...
         */
        private getFunction(name: string, fallback: Function = () => true) {
         ...
         */
        private storeFunction(name: string, fnc: Function = () => true) {
         ...
         */
        protected hasFunction(name: string) {
         ...
         */
        private handleThrottle(client: Client) {
         ...
         */
        disable() {
            debug(DEBUGLEVEL.VERBOSE)(`Command "${this.getCommandName()}" has been disabled`)
            this._enabled = false
            return this
        }
         ...
         */
        enable() {
            debug(DEBUGLEVEL.VERBOSE)(`Command "${this.getCommandName()}" has been enabled`)
            this._enabled = true
            return this
        }
         ...
         */
        dispatchCommand(args: any, ev: any) {
            return this.getFunction("exec")(ev.client, args, getReplyOutput(ev), ev)
        }
         ...
         * @returns {Command} returns this to chain Functions
         */
        checkPermission(fnc: Function) {
            this.storeFunction("perms", fnc)
            return this
        }
        ...
        isAllowed(client: Client) {
            try {
                return Boolean(this.getFunction("perms")(client))
            } catch (e) {
                debug(DEBUGLEVEL.ERROR)(e.stack)
                return false
            }
        }
        ...
        validateArgs(args: string) {
            args = args.trim()
            const resolved: Record<string, any> = {}
            const possibleErrors: Array<ParseError> = []
    ...
    class CommandGroup extends Command {
        private _cmds: Array<SubCommand> = []
        constructor(cmd: string) {
            super(cmd)
        }
         ...
         * @return
         */
        getAvailableSubCommands(client?: Client, cmd?: string) {
        ...
        isAllowed(client: Client) {
            ...
            if (super.hasFunction("exec")) return true
        ...
        run(args: string, ev: any) {
            ...
            if (sub.length === 0 && super.hasFunction("exec")) return super.dispatchCommand({}, ev)
     ...
     */
    function createCommand(cmd: string, OVERRIDES?: string) {
        CommandCollector.validateCommandName(cmd, OVERRIDES === "YES_I_KNOW_THAT_I_SHOULD_NOT_USE_COMMANDS_WITH_LENGTH_OF_ONE")
        debug(DEBUGLEVEL.VERBOSE)(`registering command '${cmd}'`)
        if (collector.getCommandByName(cmd)) {
            debug(DEBUGLEVEL.WARNING)(`WARNING there is already a command with name '${cmd}' enabled!`)
            debug(DEBUGLEVEL.WARNING)(`command.js may work not as expected!`)
        }
     ...
     */
    function createCommandGroup(cmd: string, OVERRIDES?: string) {
        CommandCollector.validateCommandName(cmd, OVERRIDES === "YES_I_KNOW_THAT_I_SHOULD_NOT_USE_COMMANDS_WITH_LENGTH_OF_ONE")
        debug(DEBUGLEVEL.VERBOSE)(`registering commandGroup '${cmd}'`)
        if (collector.getCommandByName(cmd)) {
            debug(DEBUGLEVEL.WARNING)(`WARNING there is already a command with name '${cmd}' enabled!`)
            debug(DEBUGLEVEL.WARNING)(`command.js may work not as expected!`)
        }
     ...
     */
    function createArgument(type: "client"|"number"|"string"|"rest") {
        if (typeof availableArguments[type] !== "function")
            throw new Error(`Argument type not found! Available Arguments: ${Object.keys(availableArguments).join(", ")}`)
        return availableArguments[type]()
    }
     ...
     */
    function createGroupedArgument(type: "or"|"and") {
     ...
     */
    function getReplyOutput({ mode, client, channel }: chatEvent) {
        switch (mode) {
            case 1: return client.chat.bind(client)
            case 2: return channel.chat.bind(channel)
            case 3: return backend.chat.bind(backend)
            default: return (msg: string) => debug(DEBUGLEVEL.WARNING)(`WARN no reply channel set for mode ${mode}, message "${msg}" not sent!`)
        }
    }

    interface CommandHelpReply {
        filter?: string
    }
    ...
    createCommand("help")
        ...
        .exec((client: Client, { filter }: CommandHelpReply, reply: Function) => {
    ...
    createCommand("man")
        ...
        .exec((client: Client, { command, subcommand }: CommandManReply, reply: Function) => {
            const getManual = (cmd: SubCommand|Command) => {
    ...
    event.on("chat", ev => {
        ...
        if (ev.client.isSelf()) return debug(DEBUGLEVEL.VERBOSE)("Will not handle messages from myself")
        //check if it is a possible command
        if (!collector.isPossibleCommand(ev.text)) return debug(DEBUGLEVEL.VERBOSE)("No possible valid command found!")
        //get the basic command with arguments and command splitted
        const match = ev.text.match(new RegExp(`^(?<command>\\S*)\\s*(?<args>.*)\\s*$`, "s"))
        if (!match || !match.groups) throw new Error(`RegEx missmatch for "${ev.text}"`)
        ...
        commands.forEach(async cmd => {
            ...
            try {
                ...
                debug(DEBUGLEVEL.VERBOSE)(`Command "${cmd.getFullCommandName()}" finnished successfully after ${Date.now() - start}ms`)
                //catch errors, parsing errors / permission errors or anything else
            } catch (e) {
                debug(DEBUGLEVEL.VERBOSE)(`Command "${cmd.getFullCommandName()}" failed after ${Date.now() - start}ms`)
                ...
                if (e instanceof SubCommandNotFound) {
                    ...
                    if (match) {
                        ...
                        debug(DEBUGLEVEL.ERROR)(`Unhandled Script Error in Script "${script.endsWith(".js") ? script : `${script}.js`}" on line ${line} at index ${row}`)
                        debug(DEBUGLEVEL.ERROR)(`${type}: ${msg}`)
                        debug(DEBUGLEVEL.VERBOSE)(e.stack)
                    } else {
                        debug(DEBUGLEVEL.ERROR)("This is _probably_ an Error with a Script which is using command.js!")
                        debug(DEBUGLEVEL.ERROR)(e.stack)
                    }
                }
            }
